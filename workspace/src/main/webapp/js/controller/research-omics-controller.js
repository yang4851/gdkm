gdkmApp.controller('ResearchOmicsController', function($scope, $window, $timeout, notificationService, 
		ResearchOmicsFactory, FileUploadFactory, FileDownloadFactory, Upload) {
	...
	$scope.isSubmitDisabled = function() {
		...
		if(!angular.isObject($scope.model.category) || Utils.isBlank($scope.model.category['code']))
			return true;
	...
	$scope.submit = function() {
		...
		var deferred = ($scope.model.id > 0) ? ResearchOmicsFactory.changeOmics($scope.research, $scope.model) : 
							ResearchOmicsFactory.createOmics($scope.research, $scope.model);
		deferred.then(
			function(res) {
				notificationService.success($scope.message.NOTICE001_EN + "-" + res.data.registNo);
				
				$scope.$emit('onChangeResearchOmics', res.data);
				$scope.source = res.data;
				$scope.model = angular.copy(res.data);
				
				waitingDialog.hide();
				$('#research-omics-popup').modal('hide');
			}, 
			function(err) {
				waitingDialog.hide();
	...
	$scope.download = function(file) {
		if($scope.isRegistered()) {
			FileDownloadFactory.doDownloadResearchOmics($scope.model);
		} else {
	 ...
	 */
	$scope.$on('openResearchOmicsPopup', function(event, research, omics) {
		...
		if(!angular.isObject(omics) || Utils.isBlank(omics['id'])) {
			$scope.source = null;
			$scope.model = ResearchOmics();
			$scope.model.research = $scope.research; 
		} else {
			$scope.source = omics;
			$scope.model = angular.copy(omics);
		}
		
		$('#research-omics-popup').modal('show');
	});
	
	function loadResearchOmics(research, omics) {
		ResearchOmicsFactory.getOmics(research, omics.id).then(
			function(res) {
				waitingDialog.hide();
