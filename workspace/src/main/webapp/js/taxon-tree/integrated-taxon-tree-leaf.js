gdkmApp.directive('integratedLeaf', function (integratedTree, ivhTreeviewMgr,$window,taxonTreeFactory) {
        return { 
            restrict: 'AE', 
                ...
                element.on('click', function () {
                    ...
                    function loadCompleted(result) {
                        if (result.status == 200) { 
                            ...
                            if (result.data != "") {
                            	integratedTree.genNode(result.data.list, scope.node, fakechild); 
                            } else {
                                if (scope.node.children[0].type == 'DEL') {
